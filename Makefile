...
.PHONY: help build build-qikchain build-edge clean clean-data fmt test lint \
...
help:
	...
	@echo ""
	@echo "Core:"
	@echo "  make build            Build ./bin/qikchain (and polygon-edge if available)"
...
build: build-qikchain build-edge
...
build-edge:
	@if [ -x "$(POLYGON_EDGE_BIN)" ]; then \
		...
		if bash ./scripts/fetch-polygon-edge.sh && [ -d third_party/polygon-edge ]; then \
			cd third_party/polygon-edge && $(GO) build $(GOFLAGS) -o ../../bin/polygon-edge .; \
		else \
...
clean:
	...
	@rm -rf $(BUILD_DIR)/*
	@if [ "$(RESET)" = "1" ]; then \
		echo "==> RESET=1, removing .data/ibft4"; \
		rm -rf .data/ibft4; \
	fi

clean-data:
	@echo "==> Removing .data/ibft4"
	@rm -rf .data/ibft4
...
genesis-poa: build-qikchain
	...
	$(QIKCHAIN_BIN) genesis build \
		...
		--env $(ENV) \
		--chain-id $(CHAIN_ID) \
		--out-chain build/chain.json \
		--out-genesis build/genesis-eth.json
...
genesis-validate: build-qikchain
	@echo "==> Validating build/chain.json"
	$(QIKCHAIN_BIN) genesis validate --chain build/chain.json
